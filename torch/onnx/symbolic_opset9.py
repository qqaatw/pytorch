...
def pow(g, self, exponent):
    ...
    is_self_fp = symbolic_helper._is_fp(self)
    is_exponent_fp = symbolic_helper._is_fp(exponent)
    
    if not is_self_fp:
        f_dtype = "Float"
        self = g.op("Cast", self, to_i=symbolic_helper.cast_pytorch_to_onnx[f_dtype])
    if not is_exponent_fp:
        exponent = g.op(
